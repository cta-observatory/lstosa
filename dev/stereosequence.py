...
def stereosequence(args):
    ...
    history_nosuffix = join(options.directory, "sequence_{0}_{1}".format(options.tel_id, run_str))
    ...
    if level == 1:
        ...
        log.debug("Going to level {0}".format(level))
    if level == 0:
        log.debug("Job for sequence {0} finished without fatal errors".format(run_str))
    return rc
...
def superstar(run_str, history_file):
    ...
    input_card = join(cfg.get("LSTOSA", "CARDDIR"), cfg.get(options.tel_id, "SUPERSTARCONFIGCARD"))
    ...
    star_ind_LST1 = join(
        star_dir_LST1, night_subdir, "*{0}*{1}*{2}".format("LST1", run_str, root_suffix)
    )
    star_ind_LST2 = join(
        star_dir_M2, night_subdir, "*{0}*{1}*{2}".format("M2", run_str, root_suffix)
    )
    output_basename = "{0}{1}{2}".format(superstarhistogramprefix, run_str, root_suffix)
    rc = superstar_specific(
        full_mars_command,
        input_card,
        run_str,
        star_ind_LST1,
    ...
    """ Error handling """
    try:
        nonfatalrcs = [int(k) for k in cfg.get("NONFATALRCS", "SUPERSTAR").split(",")]
    except:
        nonfatalrcs = [0]
...
def superstar_specific(
    ...
    if len(LST1_star_files) == 0:
        error(tag, "No LST1 star file of the form {0} exists yet".format(star_ind_LST1), 1)
    M2_star_files = glob(star_ind_M2)
    if len(M2_star_files) == 0:
        error(tag, "No M2 star file of the form {0} exists yet".format(star_ind_M2), 1)
    ...
    try:
        ...
        error(
            tag, 'Command "{0}" failed, {1}'.format(stringify(commandargs), NameError), ValueError
        )
    else:
        outputf = glob("{0}/20*{1}*_S_*.root".format(options.directory, run_str))
        if len(outputf) != 1:
            error(tag, "No output or multiple output files found: ".format(stringify(outputf)), 2)
        ...
        report.history(
            run_str, basename(full_mars_command), None, basename(input_card), rc, history_file
        )
        return rc
...
def melibea(run_str, history_file):
    ...
    full_mars_command = join(mars_dir, mars_command)
    root_suffix = cfg.get("LSTOSA", "ROOTSUFFIX")
    input_card = join(cfg.get("LSTOSA", "CARDDIR"), cfg.get(options.tel_id, "MELIBEACONFIGCARD"))
    rftree = join(cfg.get("ST", "RANDOMFORESTMATRIXDIR"), cfg.get("ST", "RANDOMFORESTMATRIX"))
    disp1 = join(cfg.get("ST", "DISPLST1MATRIXDIR"), cfg.get("ST", "DISPMATRIX"))
    disp2 = join(cfg.get("ST", "DISPM2MATRIXDIR"), cfg.get("ST", "DISPMATRIX"))
    energy_table = join(cfg.get("ST", "ENERGYMATRIXDIR"), cfg.get("ST", "ENERGYTABLE"))
    ...
    ind = join(
        analysis_dir, night_subdir, "*{0}{1}*{2}".format(run_str, input_pattern, root_suffix)
    )
    ...
    output_basename = "{1}".format(melibeahistogramprefix, run_str, root_suffix)
    ...
    """ Error handling """
    try:
        nonfatalrcs = [int(k) for k in cfg.get("NONFATALRCS", "MELIBEA").split(",")]
    except:
        nonfatalrcs = [0]
    if rc not in nonfatalrcs:
        sys.exit(rc)
...
def melibea_specific(
    ...
    if len(superstar_files) == 0:
        error(tag, "No superstar files of the form {0} exists yet".format(ind), 1)
...
def odie(run_str, history_file):
    ...
    from utils import magicdate_to_dir, magicdate_to_number
    ...
    full_mars_command = join(mars_dir, mars_command)
    root_suffix = cfg.get("LSTOSA", "ROOTSUFFIX")
    input_card = join(cfg.get("LSTOSA", "CARDDIR"), cfg.get(options.tel_id, "ODIECONFIGCARD"))
    analysis_dir = cfg.get("ST", "ANALYSISDIR")
    ...
    output_basename = "{0}_{1}{2}{3}{4}".format(
        night_number, run_str, odiepattern, src_name, root_suffix
    )

    rc = odie_specific(
        full_mars_command, input_card, run_str, ind, out, output_basename, history_file
    )

    return 0
    """ Error handling """
    try:
        nonfatalrcs = [int(k) for k in cfg.get("NONFATALRCS", "ODIE").split(",")]
    except:
        nonfatalrcs = [0]
    if rc not in nonfatalrcs:
        sys.exit(rc)
...
#
##############################################################################
def odie_specific(full_mars_command, input_card, run_str, ind, out, output_basename, history_file):
    tag = gettag()
    # Usage of odie:
    #   odie [flag options] --config=odie.rc
    #
    ...
    if len(melibea_files) == 0:
        error(tag, "No melibea files of the form {0} exists yet".format(ind), 1)
