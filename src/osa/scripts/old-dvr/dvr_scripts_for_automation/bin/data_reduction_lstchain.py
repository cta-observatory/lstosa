...
PATH = "PATH=/fefs/aswg/software/conda/envs/lstchain-v0.11.3/bin:$PATH"
...
def get_sbatch_instruction(
    run_id, subrun, log_dir, input_file, output_dir, pixelmap_file
):
    return dedent(
        f"""             
    start=$(/usr/bin/date '+%H:%M:%S')
    echo $start
    lstchain_r0g_to_r0v -f {input_file} -o {output_dir} --pixselection-file {pixelmap_file} --log {log_dir}/dvr_{run_id:05d}_{subrun:04d}.log
...
def drafts_job_file(original_dir,output_dir,log_dir,name_job,first_subrun,run_id,subrun,write_job_file,job_file,i):
    ...
    if not os.path.exists(pixel_file):
             ...
             if not write_job_file:
                 write_job_file = True
             with open(job_file, "a") as f:
                  file_stats = os.stat(job_file)
                  if subrun == first_subrun or file_stats.st_size == 0 :  # Only write instructions for the first subrun of the run
                      f.write(get_sbatch_script(run_id, log_dir,name_job,i))
                  f.write(
                      get_sbatch_instruction(
                        run_id,
...
def apply_pixel_selection(date):
    ...
    output_basedir = Path("/fefs/onsite/data/lst-pipe/LSTN-01/R0V")
    output_dir = output_basedir / date
    log_dir = output_basedir / "log" / date
    output_dir.mkdir(parents=True, exist_ok=True)
    log_dir.mkdir(parents=True, exist_ok=True)
    original_dir = Path(f"/fefs/onsite/data/lst-pipe/LSTN-01/R0G/{date}")
    if not os.path.exists(original_dir):
            original_dir= Path (f"/fefs/onsite/data/lst-pipe/LSTN-01/R0/{date}")
